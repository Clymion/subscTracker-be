...
class TestGetMultipleExchangeRatesAPI:
    ...
    def test_get_rates_with_base_currency_returns_200(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Success] GET /exchange-rates: Returns 200 with rates for a specific base currency."""
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?base_currency=USD", headers=headers)
        data = assert_success_response(response, 200)
        assert data["data"]["base_currency"] == "USD"
        assert "JPY" in data["data"]["rates"]
        assert "EUR" not in data["data"]["rates"]  # EUR is not a target from USD in test data

    def test_get_rates_with_target_currencies_returns_200(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Success] GET /exchange-rates: Returns 200 with filtered rates for target_currencies."""
        headers = make_auth_headers()
        # Add EUR rate for USD to test filtering
        setup_exchange_rates.add(ExchangeRate(from_currency="USD", to_currency="EUR", date=date(2025, 9, 26), rate=0.92, source="test"))
        setup_exchange_rates.commit()

        response = client.get("/api/v1/exchange-rates?base_currency=USD&target_currencies=JPY,EUR", headers=headers)
        data = assert_success_response(response, 200)
        assert "JPY" in data["data"]["rates"]
        assert "EUR" in data["data"]["rates"]

    def test_get_rates_with_specific_date_returns_200(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Success] GET /exchange-rates: Returns 200 with rates for a specific date."""
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?base_currency=USD&date=2025-09-25", headers=headers)
        data = assert_success_response(response, 200)
        assert data["data"]["date"] == "2025-09-25"
        assert data["data"]["rates"]["JPY"] == 144.5

    def test_get_rates_no_params_returns_defaults_200(self, client: FlaskClient, setup_exchange_rates: Session):
    ...
    def test_get_rates_invalid_date_format_returns_400(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Failure] GET /exchange-rates: Returns 400 for invalid date format."""
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?date=invalid-date", headers=headers)
        assert_error_response(response, 400, "Invalid date format. Use YYYY-MM-DD.")

    def test_get_rates_invalid_currency_code_returns_400(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Failure] GET /exchange-rates: Returns 400 for invalid currency code."""
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?base_currency=INVALID", headers=headers)
        assert_error_response(response, 400, "Invalid currency code provided.")

    def test_get_rates_not_found_returns_404(self, client: FlaskClient, setup_exchange_rates: Session):
        """[Failure] GET /exchange-rates: Returns 404 if no rates are found."""
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?base_currency=GBP", headers=headers)
        assert_error_response(response, 404, "Exchange rates not found for the given criteria.")

    def test_get_rates_with_all_params_returns_200(self, client: FlaskClient, setup_exchange_rates: Session):
        ...
        setup_exchange_rates.add_all([
            ExchangeRate(from_currency="USD", to_currency="EUR", date=date(2025, 9, 25), rate=0.93, source="test"),
            ExchangeRate(from_currency="USD", to_currency="GBP", date=date(2025, 9, 25), rate=0.82, source="test"),
        ])
        setup_exchange_rates.commit()
        ...
        assert data["data"]["rates"]["JPY"] == 144.5
        assert data["data"]["rates"]["EUR"] == 0.93
    ...
    def test_get_rates_invalid_target_currency_returns_400(self, client: FlaskClient):
        ...
        headers = make_auth_headers()
        response = client.get("/api/v1/exchange-rates?target_currencies=INVALID", headers=headers)
        assert_error_response(response, 400, "Invalid currency code provided.")