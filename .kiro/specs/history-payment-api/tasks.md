...
- [x] 3. Service層の実装 (TDD)
- [x] 3.1 `PaymentHistoryService`のユニットテストを作成 (Red)
...
- [x] 3.2 `PaymentHistoryService`の実装 (Green)
  - `app/services/payment_history_service.py` を新規作成します。
  - タスク3.1で作成したユニットテストをパスさせるために、`get_payment_history`メソッドを実装します。
  - _Requirements: 1.1, 1.2_

- [x] 4. API層の実装 (TDD/BDD)
- [x] 4.1 `GET /api/v1/payments`エンドポイントの統合テストを作成 (Red)
- [x] 4.2 APIエンドポイントの実装 (Green)